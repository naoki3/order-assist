...
function buildBody(
  ...
  const rows: [string, string][] = [
    ...
    ['Affected Users', String(issue.userCount ?? '—')],
  ...
  const stackSection =
    exc && stackLines.length > 0
      ? ['', '## Stack Trace', '', '```', `${exc.type}: ${exc.value}`, '', stackLines.join('\n'), '```'].join('\n')
      : '';
  ...
  return [
    ...
    table,
    '',
    ...(issue.permalink ? [`**Sentry Link:** ${issue.permalink}`, ''] : []),
    stackSection,
    '',
    '---',
    `*Auto-generated from Sentry alert. Sentry Issue ID: \`${issue.id}\`*`,
  ].join('\n');
}
...
export async function POST(req: NextRequest) {
  ...
  const ghRes = await fetch(`https://api.github.com/repos/${owner}/${repo}/issues`, {
    ...
    body: JSON.stringify({
      ...
      body: buildBody(issue, payload.data?.event, payload.triggered_rule),
      labels: ['bug', 'sentry'],
    }),
  });
