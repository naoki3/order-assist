-- Fix fn_allocate_shipment_line and fn_deallocate_shipment_line to also set/clear status_color on lots
...
BEGIN
  ...
  -- 引当中ステータスをロットに設定 (status_name + status_color も含む)
  UPDATE lots
    SET status_id   = ist.id,
        status_name  = ist.name,
        status_color = ist.color
    FROM (
      SELECT ist2.id, ist2.name, ist2.color
      FROM inventory_statuses ist2
      JOIN shipments sh ON sh.user_id = ist2.user_id
      WHERE sh.id = v_line.shipment_id AND ist2.name = '引当中'
      LIMIT 1
    ) ist
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    UPDATE lots SET status_id = NULL, status_name = NULL, status_color = NULL WHERE id = v_line.lot_id;
  END IF;
